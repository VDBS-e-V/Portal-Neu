...
$label = trim((string) ($profile['display_name'] ?? $profile['person_display_name'] ?? 'Mein Profil'));
?>
...
<section class="section--page-title section--page-title-compact">
    <div class="page-title page-title--card page-title--split page-title--compact">
        <div class="page-title__main">
            ...
            <p class="page-title__lead">Eigene Stammdaten einsehen.</p>
        </div>
        <div class="page-title__side"><div class="page-title__actions btn-group"><a class="btn btn--primary" href="/konto/profil/bearbeiten">Bearbeiten</a><a class="btn btn--outline" href="/konto">Konto</a></div></div>
    </div>
</section>

<section>
    <div class="table-block table-block--card">
        <div class="table-block__header"><h2 class="table-block__title"><?= $e($label) ?></h2></div>
        <div class="table-wrapper table-wrapper--bordered">
            <table class="table table--compact table--striped">
                <tbody>
                <?php foreach ([
                    'Anzeigename' => $profile['display_name'] ?? $profile['person_display_name'] ?? '',
                    'Vorname' => $profile['first_name'] ?? '',
                    'Nachname' => $profile['last_name'] ?? '',
                    'Rufname' => $profile['preferred_name'] ?? '',
                    'Pronomen' => $profile['pronouns'] ?? '',
                    'E-Mail' => $profile['email'] ?? $profile['login_email'] ?? '',
                ] as $key => $value): ?>
