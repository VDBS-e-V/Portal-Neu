...
final class PersonRepository
{
    ...
    private function searchPersons(array $filters): array
    {
        ...
        if ($groupId > 0 && $this->tableExists('ids_person_permission_groups')) {
            $joins[] = 'INNER JOIN ids_person_permission_groups ppg_filter ON ppg_filter.person_id = p.id';
            $where[] = 'ppg_filter.permission_group_id = :group_id';
        ...
        $sql = 'SELECT
                    ...
                    u.email AS login_email,
                    u.status AS login_status,
                    GROUP_CONCAT(DISTINCT pg.group_key ORDER BY pg.group_key SEPARATOR ", ") AS group_keys
                FROM ids_persons p
                ' . implode("\n", $joins) . '
                LEFT JOIN ids_person_permission_groups ppg ON ppg.person_id = p.id
                LEFT JOIN ids_permission_groups pg ON pg.id = ppg.permission_group_id';
    ...
    private function searchLegacyUsers(array $filters): array
    {
        ...
        $sql = 'SELECT
                ...
                FROM ids_users u
                LEFT JOIN ids_user_names n ON n.user_id = u.id
                LEFT JOIN ids_user_permission_groups upg ON upg.user_id = u.id
                LEFT JOIN ids_permission_groups pg ON pg.id = upg.permission_group_id';
